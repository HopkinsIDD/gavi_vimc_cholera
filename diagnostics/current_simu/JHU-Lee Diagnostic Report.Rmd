---
title: "Diagnostic Report -- 201910gavi-5"
author: "JHU-Lee"
date: "`r Sys.time()`"
output:
...
setwd('C:/Users/ZOU/Desktop/data_to_verify')
cen_camp  <- read_csv('central-burden-template.201910gavi-5.Cholera-standard-template.306.csvcampaign-default.csv')
cen_novax <- read_csv('central-burden-template.201910gavi-5.Cholera-standard-template.306.csvno-vaccination.csv')
```
...
model_spec <- cen_camp %>% 
  ...
  mutate(SimulatedOutcomes = paste(names(cen_camp)[(ncol(cen_camp)-3):ncol(cen_camp)], collapse='  ') ) %>% 
...
cov_novax <- read_csv("cov_novax.csv")
cov_camp <- read_csv("cov_camp.csv")
...
### plotting
cov_camp %>%
  filter(country_code %in% c('COD', 'KEN', 'HTI')) %>% 
...
```{r echo=FALSE}
outcome_code = rep(names(cen_camp)[(ncol(cen_camp)-3):ncol(cen_camp)], 2)

kable(data.frame(scenario_description = c(rep('Campaign, Default', 4), rep('No vaccination', 4)),
                 outcome_code = outcome_code,
                 number_of_non_na_rows =  c(colSums(!is.na(cen_camp[outcome_code[1:4]])),
                                            colSums(!is.na(cen_novax[outcome_code[5:8]])) )    ))
rm(outcome_code)
...
stack_camp <- cen_camp
stack_camp$id <- 1:nrow(stack_camp)
stack_camp$source <- 'Campaign, Default'
stack_novax <- cen_novax
stack_novax$id <- 1:nrow(stack_novax)
...
stacked <- stacked %>% 
  #mutate(cases1, cases2, cohort_size1, cohort_size2, dalys1, dalys2, deaths1, deaths2) %>% 
  group_by(year, age, source) %>% 
  mutate( cases1 = case_when(sum(!is.na(cases))<5 ~ 0,
                             sum(!is.na(cases))==5 ~ 1) ) %>% 
  mutate( cases2 = sum(cases > 0) ) %>% 
  mutate( cohort_size1 = case_when(sum(!is.na(cohort_size))<5 ~ 0,
                             sum(!is.na(cohort_size))==5 ~ 1) ) %>% 
  mutate( cohort_size2 = sum(cohort_size > 0) ) %>%
  mutate( dalys1 = case_when(sum(!is.na(dalys))<5 ~ 0,
                             sum(!is.na(dalys))==5 ~ 1) ) %>% 
  mutate( dalys2 = sum(dalys > 0) ) %>%
  mutate( deaths1 = case_when(sum(!is.na(deaths))<5 ~ 0,
                             sum(!is.na(deaths))==5 ~ 1) ) %>% 
...
This figure shows the shape of the upload by scenarios and outcomes (whether there are missing values)
...
### Notion: the interpolated population dataset doesn't have info for age 100, we will leave that out to keep the consistency. 
```{r include=FALSE}
### prepare the datasets
#write.csv(int_pop,"C:/Users/ZOU/Desktop/Diagnostics Report\\montagu_interpolated_pop.csv", row.names = FALSE)
int_pop <- read_csv('C:/Users/ZOU/Desktop/data_to_verify/montagu_interpolated_pop.csv')
int_pop <- int_pop %>% 
...
no_class <- pop_plot_no_country_class %>% 
  ...
  summarise(freq = n()) %>% 
  mutate(Countries = "All", PopulationDifference = population_value, Frequency = freq, Proportion = freq/(nrow(pop_plot_no_country_class)/3)) %>% 
...
all_countries <- pop_plot_all_countries %>% 
  ...
  summarise(freq = n()) %>% 
  mutate(Countries = country, PopulationDifference = population_value, Frequency = freq, Proportion = freq/(nrow(pop_plot_all_countries)/9)) %>% 
...
This figure shows the global burden values by decades 2000-2030. 
...
merged_no_country_2030 <- merged_no_country_class %>% 
  ...
  filter(age == 0, year == 2000 | year == 2011 | year == 2021) %>% 
...
This figure shows the global burden disease values 2000-2100. 
```{r echo=FALSE, fig.width=10, fig.height=10}
### prepare the dataset and plot
