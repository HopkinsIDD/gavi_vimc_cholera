...
#' @export 
load_shapefile_by_country <- function(datapath, country){

  if (country %in% c("COD", "ETH", "KEN", "SOM", "SSD")){
    shp_fn <- list.files(path = paste0(datapath, "/shapefiles"), pattern = paste(country, "2", sep = "_"))
    if (length(shp_fn) > 1){
      stop(paste("There is more than one shapefile with", paste(country, "2", sep = "_"), "in the filename"))
    }
