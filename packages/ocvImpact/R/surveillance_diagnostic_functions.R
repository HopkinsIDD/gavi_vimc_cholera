...
combine_output <- function(cache, output_to_combine, save_final_output = TRUE){
  ...
  if("target_table" %in% output_to_combine){
    ...
    for(country in names(cache)){
      for(gen_scn in c("campaign-default", "no-vaccination")){ #first get all the campaign data in
        for(fn in cache[[country]]$target_table_fns){
          ...
          fn_in_list <- unlist(strsplit(fn, split = "_"))
    ...
    # # restructure the target table ***
    # target_table_spreaded <- target_table %>% 
    #   filter(admin_level == "admin1") %>%
    #   select(- NAME_2) %>% 
    #   mutate(general_scenario = gsub("-", "_", general_scenario)) %>% 
    #   mutate(general_scenario = paste0(general_scenario, "_true_case")) %>% 
    #   tidyr::spread(general_scenario, true_case)

    # save the table in the final output folder 
    if(save_final_output){
      runname <- unlist(strsplit(fn, "/"))[2]
...
plot_cases <- function(cache, case_type, threshold, cumulative_type){
  ...
  if(grepl("district", cumulative_type) & grepl("true", case_type)){
    ...
    plt <- plt_table %>% 
      ggplot() +
      geom_bar(data = plt_table, 
              aes(x=NAME_1, y=averted_observed_case), 
              stat = "identity", color = "black", size = 0.2, fill = "orange") +
      facet_wrap( ISO + admin_level ~ surveillance_scenario, ncol = 3) +
      theme_minimal() + 
      coord_flip()
...
plot_efficacy <- function(cache, compare_type, threshold, cumulative_type){
  ...
  # Just for side-by-side comparison -- consider add na.rm = T to the sum() and mean() because it's inevitable
