...
import_coverage_scenario <- function(modelpath, country, scenario, cache, filter0 = FALSE, redownload = TRUE){

  ## look for whether the coverage scenario was already read in
  if(!is.null(cache[["coverage_scenario"]])){
    message("Loading montagu cache: coverage_scenario")
    cov_dat <- cache[["coverage_scenario"]]

  } else{
    #First check, then retrieve
    CoverageFiles <- list.files(modelpath, pattern = "^coverage_")
    if (length(CoverageFiles) >= 3 & redownload == FALSE){ #there should be 3 coverage data files, so the default should be 3
...
import_centralburden_template <- function(modelpath, country, cache, redownload = TRUE){

  if(!is.null(cache[["centralburden_template"]])){
    message("Loading montagu cache: centralburden_template")
    rc <- cache[["centralburden_template"]]
...
import_country_population <- function(modelpath, country, cache, redownload = TRUE){

  if(!is.null(cache[["country_population"]])){
    message("Loading montagu cache: country_population")
    rc <- cache[["country_population"]]
...
#' @name import_int_country_population
#' @title import_int_country_population
#' @description Get 1-year country population from standardized source in Montagu. [may be used in future for case-based targeting]
#' @param modelpath path to montagu files
#' @param country country code
#' @param cache montagu cache
#' @param redownload whether to redownload the file
...
#' @export
#' @include retrieve_montagu_population.R
import_int_country_population <- function(modelpath, country, cache, redownload = TRUE){

  if(!is.null(cache[["int_country_population"]])){
    message("Loading montagu cache: int_country_population")
    rc <- cache[["int_country_population"]]
...
#' @export
import_country_population_1yr <- function(modelpath, country, year, cache){

  country_pop <- import_country_population(modelpath, country, cache, redownload = FALSE) #this is important
...
import_country_agePop <- function(modelpath, country, cache, redownload = TRUE){

  if(!is.null(cache[["country_agePop"]])){
    message("Loading montagu cache: country_agePop")
    rc <- cache[["country_agePop"]]

  } else{

    if(is.null(cache[["import_int_country_population"]])){
      cache[["import_int_country_population"]] <- import_int_country_population(modelpath, country, cache, redownload = FALSE)
    }
    agepop <- cache[["import_int_country_population"]] %>%
      dplyr::filter(country_code == !!country) %>%
...
import_country_lifeExpectancy <- function(modelpath, country, cache, redownload = TRUE){

  if(!is.null(cache[["country_lifeExpectancy"]])){
    message("Loading montagu cache: country_lifeExpectancy")
    rc <- cache[["country_lifeExpectancy"]]
...
#' @return dataframe
#' @export
import_country_lifeExpectancy_1yr <- function(modelpath, country, year, cache){
  lx0_df <- import_country_lifeExpectancy(modelpath, country, cache, redownload = FALSE) #important change
...
#' @export
#' @include retrieve_montagu_agePop.R
import_country_proportion_under5 <- function(modelpath, country, year, cache, redownload = TRUE){
...
adjusted_montagu_coverage <- function(coverage_sheet, cntrycode){
  ...
  for (i in 1:nrow(df)){
    if (df[i,]$vaccine == 'OCV1' & any(df$vaccine == 'OCV2' & df$year == df[i,]$year)){
      ...
      df[i,]$new_coverage <- (ocv1_coverage + ocv2_coverage - 2 * (ocv1_coverage*ocv2_coverage)) ## elizabeth's formula for ocv1 coverage
