...
update_sus_rasterStack_optimized <- function( datapath, 
  ...
  raster1_template <- terra::app(pop, fun = function(x){ifelse(!is.na(x), 1, NA)})
  ...
  if(scenario == 'campaign-default'){
    ...
    for (year in max(baseline_year, model_year-last_vac_ef_year):model_year){
      ...
      popk <- terra::rast(vac_pop_fn)
      if("rc1" %in% rc_targeted){vacck_admin1 <- terra::rast(vac_admin1_fn)}
      if("rc2" %in% rc_targeted){vacck_admin2 <- terra::rast(vac_admin2_fn)}
      ...
      popk[popk == 0] <- 1
      popj[popj == 0] <- 1
      pkj <- terra::lapp(c(popk, popj), fun = function(x, y){x * (1 - ((j - k) * mu)) / y})
      
      ve_j_k <- as.numeric(ve_direct(j-k)) #if same year, change it from 1 to 0
        
      if("rc1" %in% rc_targeted){prob_still_protected_admin1 <- terra::lapp(c(vacck_admin1, pkj), fun = function(x, y){x * y * ve_j_k})}
...
save_sus_raster <- function(datapath, modelpath, country, nsamples, model_year, sus_list, 
  ...
  if(!is.null(sus_list) & is.null(names(sus_list)) & !is.null(rawoutpath)){ #if there's no name, it means the update_sus_rasterStack function is used
    ...
    if( !file.exists(sus1_out_fn) | (file.exists(sus1_out_fn)&clean) ){
      if(!is.null(sus_admin1)){terra::writeRaster(sus_admin1, filename = sus1_out_fn, overwrite = TRUE)}}
    if( !file.exists(sus2_out_fn) | (file.exists(sus2_out_fn)&clean) ){
      if(!is.null(sus_admin2)){terra::writeRaster(sus_admin2, filename = sus2_out_fn, overwrite = TRUE)}}
  ...
  if(is.null(rawoutpath)){ #must save the sus raster in the intermediate folder each year during the simulation
    ...
    if(!is.null(sus_list$sus_rasterStack_admin1)){terra::writeRaster(sus_list$sus_rasterStack_admin1, filename = sus1_inter_fn, overwrite = TRUE)}
    if(!is.null(sus_list$sus_rasterStack_admin2)){terra::writeRaster(sus_list$sus_rasterStack_admin2, filename = sus2_inter_fn, overwrite = TRUE)}
...
update_sus_rasterStack <- function(datapath, 
  ...
  for(k in 1:j){ #may not need that many years given the protection of vaccines is not to last for more than 5 years 
    ...
    vacck_admin1 <- vacc_rasterStack_admin1[[k]]
    vacck_admin2 <- vacc_rasterStack_admin2[[k]]
    ...
    prob_still_protected_admin1 <- terra::lapp(c(vacck_admin1, pkj), fun = function(x,y){
      x * y * ve_j_k
    })
    prob_still_protected_admin2 <- terra::lapp(c(vacck_admin2, pkj), fun = function(x, y){
      x * y * ve_j_k
    })
      
    # get the new sus raster layer -- from protected to still susceptible 
    tmp1 <- terra::lapp(c(tmp1, prob_still_protected_admin1), fun = function(x, y){
      x * (1 - y)
    })
    tmp2 <- terra::lapp(c(tmp2, prob_still_protected_admin2), fun = function(x, y){
      x * (1 - y)
    })
  ...
  if(is.null(sus_list)){
    ...
    sus_rasterStack_admin1 <- c(sus_rasterStack_admin1, tmp1)
    sus_rasterStack_admin2 <- c(sus_rasterStack_admin2, tmp2)
  }
  rm(tmp1, tmp2, popj)
  gc()
