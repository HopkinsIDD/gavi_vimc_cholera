...
n_targeted_admins <- read.csv("data/n_targeted_admins_allISOs.csv")
...
n_admins <- n_targeted_admins %>%
  ...
  dplyr::select(confirmation_lens, admin_level, threshold, n_total_targets, n_unique_targets)
```
...
tab <- df_tp_eff_allISOs %>%
  ...
  left_join(n_admins, by = c("threshold", "confirmation_lens", "admin_level")) %>%
  dplyr::select(threshold, confirmation_lens, admin_level, tp_cumu, ac, efficiency, n_total_targets, n_unique_targets) %>%
  pivot_wider(names_from = "confirmation_lens", values_from = c("tp_cumu", "ac", "efficiency", "n_total_targets", "n_unique_targets")) %>%
  ...
  set_header_labels(threshold = "", admin_level = "",
                    ...
                    `n_total_targets_district-estimate` = "District labs", `n_total_targets_global-estimate` = "National lab", `n_total_targets_no-estimate` = "Clinical only",
                    `n_unique_targets_district-estimate` = "District labs", `n_unique_targets_global-estimate` = "National lab", `n_unique_targets_no-estimate` = "Clinical only") %>%
  # merge cells 
  ...
  vline(part = "all", j = 11) %>%
  vline(part = "all", j = 14) %>%
  hline(part = "body", i = 2, j = 2:17) %>%
  hline(part = "body", i = 4, j = 2:17) %>%
  hline(part = "body", i = 1:4, j = 1) 
  
tab

# save the main table
save_as_docx(tab, path = "tables/table1_median.docx")
```

<br>

### Figure 1 Comparison of OCV efficiency across model scenarios
* Each of the dot in the boxplot represents the **median** efficiency of each country.
```{r efficiency ratio figure}
df_ratio <- df_raw %>% filter(year == 2030) %>%
  ...
  mutate(type = case_when(type == "globalno" ~ "Clinical only→ \nNational lab",
                          type == "districtno" ~ "Clinical only→ \nDistrict labs",
...
fig1 <- cowplot::plot_grid(fig_eff_boxplot, fig_eff_ratio,
                   labels = c('A', 'B'),
                   nrow = 1)

fig1
ggsave(plot = fig1, filename = "figures/fig1.pdf", height = 6, width = 12)
```

<br>

### Figure 2 OCV efficiency gained from cholera confirmation at finer scale
* Figure 2A shows difference between **median** efficiency of "Clinical only" and "District labs";
* The maps (figure 2B) are showing number of doses administered when doing admin2 targeting and district level confirmation of cholera (district labs); 
...
map <- readRDS("figures/dose_maps_0.001_1e-04.rds") + theme(plot.margin = unit(c(-8, 0, -2, 5), "cm"))
...
fig2 <- plot_grid(fig, map, labels = c("", "B"), ncol = 1, rel_heights = c(1,1.4), label_size = 20)

fig2

ggsave(plot = fig2, filename = "figures/fig2.pdf", height = 14, width = 12)
```
...
### Figure S2 Gained efficiency from "Clinical only" to "District labs" (Threshold = 2/10,000 per year)
```{r fig.height=6, fig.width=12, out.width='100%'}
p2 <- plot_gained_eff_with_size(df_dose_eff_median, threshold_chosen = 2e-04,
                baseline_estimate = "no-estimate", improved_estimate = "district-estimate") + 
  theme(legend.position = "bottom", legend.box = "vertical")

map <- readRDS("figures/dose_maps_no-district_2e-04.rds") + theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))

#cowplot::plot_grid(p2, map, labels = c("A", "B"), ncol = 1, rel_heights = c(1,1.2))
figS2 <- cowplot::plot_grid(p2, map, labels = c("A", "B"), nrow = 1)
figS2

ggsave(plot = figS2, filename = "figures/figS2.pdf", height = 6, width = 12)
```

<br>

### Figure S3 Gained efficiency from "Clinical only" to "National lab"
...
p123 <- plot_grid(plotlist = list(p1,p2,p3), nrow = 1)
fig_gained_eff_no_global <- plot_grid(plotlist = list(leg, p123), nrow = 2, rel_heights = c(1,15))

figS3 <- fig_gained_eff_no_global
figS3

ggsave(plot = figS3, filename = "figures/figS3.pdf", height = 6, width = 20)
```

<br>

### Figure S4 Gained efficiency from "National lab" to "District labs"
```{r fig.height=6, fig.width=20}
p1 <- plot_gained_eff_with_size(df_dose_eff_median, threshold_chosen = 0.001,
...
figS4 <- fig_gained_eff_global_district
figS4

ggsave(plot = figS4, filename = "figures/figS4.pdf", height = 6, width = 20)
```

<br>

### Table S1 Districts excluded from targeting due to small size
...
tabS1 <- 
  ...
  flextable() %>%
  # set the header values
  add_header_row(top = TRUE,values = c("ISO", "Admin1", "Admin2", "Population Size", "Incidence Rate(per million)", "Area (km^2)", "Cholera confirmation scenarios","","" ,"Threshold scenarios", "","")) %>%
  # merge cells 
...
# save the main table
save_as_docx(tabS1, path = "tables/tableS1.docx")
```

<br>

### Table S2 Targeted population, averted cases and OCV efficiency of each country
...
# save the main table
save_as_docx(tab_byISO, path = "tables/tableS2_median.docx")
```
