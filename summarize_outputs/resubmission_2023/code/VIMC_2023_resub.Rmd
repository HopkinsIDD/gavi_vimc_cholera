---
title: "Enhanced cholera surveillance as a tool for improving vaccination campaign efficiency (2023 resubmission)"
...
n_targeted_admins_allISOs <- read.csv("data/n_targeted_admins_allISOs.csv")
n_targeted_ISOs <- read.csv("data/n_targeted_admins_ISOs.csv")
...
n_ISOs <- n_targeted_ISOs %>%
  dplyr::select(confirmation_lens, admin_level, threshold, 
                n_targeted_ISO_lb, n_targeted_ISO_median, n_targeted_ISO_ub)
  

n_admins_ISOs <- n_targeted_admins_allISOs %>%
  left_join(n_ISOs, by = c("confirmation_lens", "admin_level", "threshold")) %>%
...
table3 <- cost_spend_save_per_ac_allISOs %>%
  filter(admin_level == "admin2") %>% 
  ...
  add_header_row(top = TRUE,values = c("Threshold",  
                                       "Total cost per averted case ($)", "",  
                                       "OCV cost reduction per averted case ($)", "Test cost per averted case ($)", "OCV cost reduction per test dollar spent ($)")) %>%
  set_header_labels(threshold = "Threshold",
                    ...
                    ocv_cost_saved_per_ac = "OCV cost reduction per averted case ($)", test_cost_per_ac = "Test cost per averted case ($)",
                    spend_cost_ratio = "OCV cost reduction per test dollar spent ($)") %>%
...
plt.compare.ac <- df.compare %>%
  ...
  theme(legend.key.size = unit(1, 'cm'), #legend key size
        legend.title = element_text(size=15), #legend title font size
...
# figure 2B total cost (by country)
...
plt.spend.save <-
  ...
  geom_segment(data = df.lines, 
    ...
    xlab("Test cost per averted case ($)") + ylab("OCV cost reduction per averted case ($)") + 
    scale_color_brewer(palette="Set2") +
  ...
  geom_text(data = df.lines, 
            aes(x = test_threshold_10, 
                y = ocv_threshold_10, 
                label = countryname), size = 4, nudge_y = -0.03, nudge_x = 0.08, color = "grey33")  +
...
ggsave(plot = plt.compare.fvp, filename = "figures/figS3.jpg", height = 6, width = 8)

```

```{r}
# get the estimate for an individual country i.e. Nigeria
cost_spend_save_per_ac_byISO %>%
  filter(ISO == "NGA") %>% 
  filter(admin_level == "admin2") %>%
...
ggsave(plot = plt.vaxed.boxplot, filename = "figures/fig3.jpg", height = 6, width = 6)

```

<br>

```{r fig.width=14, fig.height = 6, eval = F}
...
Get the ratios (in the draft) of fvp, ac, vaccine efficiency between "Clinical definitions" and "Decentralized testing":
```{r }
## to get the numbers in the draft about ratio between fvp, ac and vaccine efficiency
# OCV ratio
df_ratio %>% filter(type == "Clinical Definition→ \nDecentral. Testing") %>%
  filter(admin_level == "admin2")

# fvp ratio and averted case ratio
df_raw %>% filter(admin_level == "admin2") %>%
  filter(confirmation_lens != "global-estimate") %>%
  filter(year == 2035) %>%
  dplyr::select(-year, -admin_level) %>%
  pivot_wider(names_from = "confirmation_lens", values_from = c("target_pop_cumu", "true_ac_cumu")) %>%
  group_by(threshold) %>% 
  summarize(fvp_ratio = `target_pop_cumu_no-estimate` /  `target_pop_cumu_district-estimate`,
            ac_ratio = `true_ac_cumu_no-estimate` / `true_ac_cumu_district-estimate`) %>%
  group_by(threshold) %>% 
  summarize(fvp_ratio_median = quantile(fvp_ratio, 0.5, na.rm = T),
            fvp_ratio_lb = quantile(fvp_ratio, 0.025, na.rm = T),
            fvp_ratio_ub = quantile(fvp_ratio, 0.975, na.rm = T),
            ac_ratio_median = quantile(ac_ratio, 0.5, na.rm = T), 
            ac_ratio_lb = quantile(ac_ratio, 0.025, na.rm = T),
            ac_ratio_ub = quantile(ac_ratio, 0.975, na.rm = T))
```
...
figS2 <- cowplot::plot_grid(fig_eff_ratio,fig_eff_boxplot,
                   labels = c('A', 'B'),
                   nrow = 1)
figS2

ggsave(plot = figS2, filename = "figures/figS2.jpg", height = 6, width = 12)
```
...
save_as_docx(tab.complete, path = "tables/tableS8.docx")
```
...
save_as_docx(table_small, path = "tables/tableS7.docx")

```


### **Figure S`r fig.counter`** Baseline incidence rate of targeted districts
```{r}
fig.counter <- fig.counter + 1
...
plt_true_ir
ggsave(plot = plt_true_ir, filename = "figures/figS4.jpg", height = 6, width = 8)
```

<br>

### **Figure S`r fig.counter`** Country level gain of OCV efficiency (Clinical definition --> decentralized testing)
```{r, fig.width = 14, fig.height = 18}
fig.counter <- fig.counter + 1

# AB plot: country level, cost effectiveness ~ dose administered
df_dose_eff_median <- df_tp_eff %>%
  mutate(doses = tp_cumu_median / 1000000 * 2) %>%
  dplyr::select(ISO, threshold, confirmation_lens, admin_level, efficiency_median, doses, ac_median) %>%
  rename(efficiency = efficiency_median)

plt.effgain.a <- plot_gained_eff_over_doses(df_dose_eff_median,
                          threshold_chosen = 0.001,
                          baseline_estimate = "no-estimate", 
                          improved_estimate = "district-estimate") + theme(legend.position = "none") 
plt.effgain.b <- plot_gained_eff_over_doses(df_dose_eff_median,
                          threshold_chosen = 2e-04,
                          baseline_estimate = "no-estimate", 
                          improved_estimate = "district-estimate") + theme(legend.position = "none") 
plt.w.legend.dose <- plot_gained_eff_over_doses(df_dose_eff_median,
                          threshold_chosen = 2e-04,
                          baseline_estimate = "no-estimate", 
                          improved_estimate = "district-estimate") + theme(legend.box="horizontal", legend.position = "bottom")
leg.dose <- get_legend(plt.w.legend.dose)
plt.AB <- plot_grid(
  plot_grid(plt.effgain.a, plt.effgain.b, ncol = 2, labels = c("A","B"), label_size = 20),
  leg.dose, ncol = 1, rel_heights = c(20, 1))


# CD plot: country level, cost effectiveness ~ doses
df_costeff <- cost_spend_save_per_ac_byISO %>%
  mutate(`costeff_district-estimate` = ocv_cost_per_ac_district_estimate_median + test_cost_per_ac_median,
         `costeff_no-estimate` = ocv_cost_per_ac_no_estimate_median) %>%
  dplyr::select(ISO, threshold, admin_level, `costeff_district-estimate`, `costeff_no-estimate`) %>%
  mutate(costeff_reduction = `costeff_no-estimate` - `costeff_district-estimate`)

plt.effgain.c <- plot_costeff_over_doses(df.costeff = df_costeff, df.doses = df_dose_eff_median ,
                                         threshold_chosen = 0.001) + theme(legend.position = "none")
plt.effgain.d <- plot_costeff_over_doses(df.costeff = df_costeff, df.doses = df_dose_eff_median ,
                                         threshold_chosen = 2e-04) + theme(legend.position = "none")
plt.w.legend.costeff <- plt.effgain.d + theme(legend.position = "bottom")
leg.costeff <- get_legend(plt.w.legend.costeff)
plt.CD <- plot_grid(
  plot_grid(plt.effgain.c, plt.effgain.d, ncol = 2, labels = c("C","D"), label_size = 20),
  leg.costeff, ncol = 1, rel_heights = c(20, 1))

# EF plot: horizontal barplot showing OCV cost reduction per test spent (currently we only calculated and )
df.cost.ratio <- cost_spend_save_per_ac_byISO %>%
  filter(admin_level == "admin2" & threshold != 1e-04) %>%
  dplyr::select(ISO, threshold, spend_cost_ratio_median) %>%
  filter(!is.na((spend_cost_ratio_median)))

plt.cost.ratio.e <- plot_cost_ratio(df.cost.ratio, threshold_chosen = 0.001)
plt.cost.ratio.f <- plot_cost_ratio(df.cost.ratio, threshold_chosen = 2e-04)
plt.EF <- plot_grid(plt.cost.ratio.e, plt.cost.ratio.f, ncol = 2, labels = c("E", "F"), label_size = 20)


plt.country.level <- plot_grid(plt.AB, plt.CD, plt.EF, nrow = 3)

ggsave(plot = plt.country.level, filename = "figures/figS5.jpg", height = 18, width = 14)
...
```{r}
# check for NGA, MWI, SSD for the spend and save numbers 
temp <-  cost_spend_save_per_ac_byISO %>% filter(ISO %in% c("NGA", "MWI", "SSD")) %>%
  filter(admin_level == "admin2") %>%
  dplyr::select(ISO, threshold, test_cost_per_ac_median, ocv_cost_per_ac_district_estimate_median) %>%
  filter(threshold != 2e-04)
```
