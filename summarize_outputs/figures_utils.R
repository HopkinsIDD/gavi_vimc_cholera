...
make_boxplot <- function(df_sum, 
  ...
  # rename
  df_sum <- df_sum %>%
    mutate(confirmation_lens = case_when(confirmation_lens == "no-estimate" ~ "Clinical only",
                                         confirmation_lens == "global-estimate" ~ "National labs",
                                         confirmation_lens == "district-estimate" ~ "Regional labs")) %>%
...
plot_gained_eff_with_size <- function(df_sum,
  ...
  # legend labels
  if(baseline_estimate == "no-estimate"){leg_labs_baseline <- "Clinical only"}
  ...
  plt <- plt +
    ...
    labs(size = "Doses administered 2022-2030 (million)")
...
plot_eff_over_inc <- function(df_sum, 
  ...
  plt <- df_sum %>%
    ...
    theme_bw() +
    xlab("National Baseline Incidence Rate") + 
    ylab("Efficiency") +
    labs(color = "Threshold")
